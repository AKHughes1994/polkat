...
def main():
    ...
    intervals_dir = f"{cfg.INTERVALS}/{filename_targetname}"
    if not os.path.exists(intervals_dir):
        os.makedirs(intervals_dir)
    
    # Create temp directory following SNAP.py convention
    OVERRIDE_TEMP_DIR = '/mnt/scratchssd'
    if OVERRIDE_TEMP_DIR == '':
        temp_dir = f"{cfg.INTERVALS}/{filename_targetname}_temp"
    else:
    ...
    # Determine if this is a calibrator (has "scan" in MS name)
    is_calibrator = 'scan' in myms
    
    # Set maxchannels: 128 for calibrators, otherwise use default from config
    maxchannels = 128 if is_calibrator else cfg.WSC_MAX_CHANNELS
    chanout = 64 if is_calibrator else cfg.WSC_SNAPSNAP_CHANNELSOUT
    ...
    for ints in int_arr:
        ...
        # Temporary name for scan image (in target-specific subdirectory)
        image_prefix = intervals_dir+f'/img_{myms}_modelsub_scan{scan_n:04d}'
        ...
        # Build list of rename pairs for this scan
