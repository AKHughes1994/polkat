...
def main():
    ...
    WHICH_MS = cfg.CAL_1GC_WHICH_MODE
    ...
    for tt in range(0,len(target_ids)):
        ...
        if targetname not in prefields_info['field_names']:
            ...
            syscall += gen.generate_syscall_wsclean(mslist = [myms],
                    ...
                    datacol = 'CORRECTED_DATA',
                    mask = img_prefix+'-MFS-image.mask0.fits',
                    automask = False,
                    chanout = 8,
                    field=targetindex,
                    pol='IQUV',
            ...
            syscall += gen.generate_syscall_casa(casascript=cfg.OXKAT+'/casa_source_pcal.py {} {} int\n\n'.format(pcal,targetname))
            step['syscall'] = syscall
            steps.append(step)
            step_i += 1
            ...
            step['pbs_config'] = cfg.PBS_WSCLEAN
            syscall = CONTAINER_RUNNER + CASA_CONTAINER+' ' if USE_SINGULARITY else ''
            syscall += gen.generate_syscall_casa(casascript=cfg.TOOLS+'/make_p_images.py')
            step['syscall'] = syscall
            steps.append(step)
            step_i += 1
