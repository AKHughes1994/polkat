...
Takes a source name and optional cutout size as command-line arguments.
...
def main():
    ...
    if len(sys.argv) < 2:
        print('Usage: python3 make_cutouts_alt.py <source_name> [cutout_size]')
        print('  source_name: Name of the source to match in filenames')
        print('  cutout_size: Optional cutout size in pixels (default: 512)')
        sys.exit(1)

    source_name = sys.argv[1]
    cutout_size = int(sys.argv[2]) if len(sys.argv) > 2 else 512
    ...
    # Collect all files across all identifier/suffix combinations
    all_jobs = []
    ...
    msg(f'Cutout size: {cutout_size} px')
