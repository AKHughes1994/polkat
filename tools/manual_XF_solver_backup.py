...
def plot_stokes_histograms(I_avg, Q_avg, U_avg, V_avg, output_dir):
    ...
    plt.savefig(os.path.join(output_dir, 'uv_avg_hist.png'))
    plt.close(fig)


def plot_stokes_spectra(freq, I_flux, Q_flux, U_flux, V_flux, output_dir):
    """
    ...
    ax[0].plot(freq, I_flux, marker='.', linestyle='none')
    ax[0].axhline(0.0, color='gray', linestyle='--', alpha=0.7)
    ax[0].set_ylabel('Stokes I [Jy]')
    ax[0].set_title('Per-channel I (vector-averaged, real part)')
    
    ax[1].plot(freq, Q_flux, marker='.', linestyle='none')
    ax[1].axhline(0.0, color='gray', linestyle='--', alpha=0.7)
    ax[1].set_ylabel('Stokes Q [Jy]')
    ax[1].set_title('Per-channel Q (vector-averaged, real part)')
    
    ax[2].plot(freq, U_flux, marker='.', linestyle='none')
    ax[2].axhline(0.0, color='gray', linestyle='--', alpha=0.7)
    ax[2].set_ylabel('Stokes U [Jy]')
    ax[2].set_title('Per-channel U (vector-averaged, real part)')
    
    ax[3].plot(freq, V_flux, marker='.', linestyle='none')
    ax[3].axhline(0.0, color='gray', linestyle='--', alpha=0.7)
    ax[3].set_ylabel('Stokes V [Jy]')
    ax[3].set_xlabel('Frequency [GHz]')
    
    plt.savefig(os.path.join(output_dir, 'uv_flux_spectra.png'))
    plt.close(fig)
...
plot_stokes_spectra(f, I_plot, Q_plot, U_plot, V_plot, xfdir)

np.savez(os.path.join(xfdir, 'uv_vector_avgs_per_channel.npz'),
         freq_ghz=f,
...
print(f"Saved: {xfdir}/uv_avg_hist.png, {xfdir}/uv_flux_spectra.png, {xfdir}/uv_vector_avgs_per_channel.npz")
...
if np.sum(good_mask) < 2:
    ...
    # Extract phases for interpolation
    ...
    gains_new = np.tile(complex_interp.reshape(1, -1, 1), (1, 1, n_antennas))
    ...
    print(f"✓ XF table populated with interpolated cross-hand phases: {xftab}")
