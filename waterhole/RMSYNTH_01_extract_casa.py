...
# TODO: at some point switch this environment over to modular CASA (pip-installed
# casatasks/casatools) properly, rather than casashell -- then astropy would
# actually be available and estimate_memory_per_worker_bytes could read image
# headers directly instead of going through imhead.
...
# These run in the main casashell process, so they use the bare imhead
# global exactly like the rest of the script -- no import needed here.
# =============================================================================
...
def estimate_memory_per_worker_bytes(image, casa_worker_floor_mb=350):
    ...
    info  = imhead(image, mode='list')
    shape = list(info['shape'])
    n_pix = 1
    for s in shape:
        n_pix *= int(s)
    datatype      = info.get('datatype', 'float')
    bytes_per_pix = 4 if 'float' in str(datatype).lower() else 8
    image_bytes   = 4 * n_pix * bytes_per_pix
    ...
    msg(f'  [mem-est] sample={os.path.basename(image)}  shape={shape}  '
        f'dtype={datatype}  {n_pix:,} pix x {bytes_per_pix}B/pix x4 copies '
        f'= {image_bytes / 1024**2:.1f}MB  vs  floor={floor_bytes / 1024**2:.0f}MB  '
...
def fit_channel_group_batch(jobs):
    '''
    Pool entry point: runs in a freshly spawned worker process, which does not
    inherit casashell's injected imfit/imstat/imhead/tb globals (spawn starts a
    clean interpreter; only fork would copy them). `global` + the import here
    binds them into this module's namespace for that process only, so the
    unmodified helper functions (check_position, get_imfit_values, etc.), which
    reference those names as plain globals, resolve them exactly as they do in
    the casashell-injected main process. Nothing outside this function changes.
    Processes its whole assigned chunk of channel-groups in one call, so the
    import cost above is paid once per worker, not once per channel.
    '''
    global imfit, imstat, imhead, tb
